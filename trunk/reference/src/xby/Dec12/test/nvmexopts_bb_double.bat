...
set INCLUDE=%VCINSTALLDIR%\ATLMFC\INCLUDE;%VCINSTALLDIR%\INCLUDE;%VCINSTALLDIR%\PlatformSDK\INCLUDE;%VSINSTALLDIR%\SDK\v2.0\include;%INCLUDE%;C:\Dokumente und Einstellungen\All Users\Anwendungsdaten\NVIDIA Corporation\NVIDIA GPU Computing SDK\C\common\inc;
set LIB=%VCINSTALLDIR%\ATLMFC\LIB;%VCINSTALLDIR%\LIB;%VCINSTALLDIR%\PlatformSDK\lib;%VSINSTALLDIR%\SDK\v2.0\lib;%MATLAB%\extern\lib\win32;C:\Programme\Microsoft SDKs\Windows\v6.0A\Lib;%LIB%;C:\Dokumente und Einstellungen\All Users\Anwendungsdaten\NVIDIA Corporation\NVIDIA GPU Computing SDK\C\common\lib;
...
rem set COMPILER=cl
set COMPILER=nvcc
rem set COMPFLAGS= -c -Xcompiler "/c /Zp8 /GR /W3 /EHs /D_CRT_SECURE_NO_DEPRECATE /D_SCL_SECURE_NO_DEPRECATE /D_SECURE_SCL=0 /DMATLAB_MEX_FILE /nologo /MD /DARCH_SM13"
set COMPFLAGS= -code=sm_13 -arch=compute_13 -c -Xcompiler "/c /Zp8 /GR /W3 /EHs /D_CRT_SECURE_NO_DEPRECATE /D_SCL_SECURE_NO_DEPRECATE /D_SECURE_SCL=0 /DMATLAB_MEX_FILE /nologo /MD"
