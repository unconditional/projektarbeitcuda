#include "cuda.h"
#include "mex.h"
__global__ void norm_elements(float* pIn, float* pOut, int N)
{
 		 int idx = blockIdx.x*blockDim.x+threadIdx.x;
 		 if(idx < N)
		 {
		  		pOut[idx] += pIn[idx]*pIn[idx];
 		 }
...
void mexFunction(int outArraySize, mxArray *pOutArray[], int inArraySize, const mxArray *pInArray[])
{
     ...
     float *data1f_gpu, *data2f_gpu;
 	 ...
 	 pOutArray[0] = mxCreateDoubleMatrix(m,n,mxREAL);
     ...
     data1 = mxGetPr(pInArray[0]);
	  ...
	  norm_elements<<<dimGrid, dimBlock>>>(data1f_gpu, data2f_gpu, n*m);
