...
__global__ void matrixMul_kernel( t_ve* C, t_ve* A, t_ve* B, int mA, int nB)
{
	...
	for(int a = aBegin; (a < aEnd)&&((gridIndex*gridDim.x+blockIdx.x)<aEnd); a += aStep, gridIndex++){
		...
		for(int b = bBegin; b < bEnd ; b += bStep ) {
			...
			Cs[threadIdx.x] = 0;
			////__syncthreads();
			// compute scalar product&&((b+threadIdx.x) < bEnd)

			if ( (threadIdx.x + b) < bEnd) {

			    if (( (gridIndex*gridDim.x+blockIdx.x)<aEnd)) {
