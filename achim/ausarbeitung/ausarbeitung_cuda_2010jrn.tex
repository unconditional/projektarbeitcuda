...
mit Feldtheoretischen Problemen. Grafikkarten (GPU) mit vielen parallelen Prozessoren
sind geeignet als Rechenhardware um die rechnergestützte Lösung dieser LGS zu beschleunigen.
CUDA ist ein API um Software für die Ausführung auf GPUs des Herstellers nvidia entwickeln zu können.
...
der linearen Algebra, im einzelnen
...
\begin{tabular}[ht]{|l|c|l|}
  ...
  \hline
  norm & $  n = |\vec{\bf{a}}|= \sqrt{ \vec{\bf{a}} \cdot \vec{\bf{a}} }$ & \\
  \hline
  matrixmul & $   \vec{\bf{c}}= \bf{A}_{full} \cdot \vec{b} $ & $\bf{A_{full}}$ vollbesetzt\\
  \hline

  sparsemul & $   \vec{\bf{c}}= \bf{A}_{sparse} \cdot \vec{b} $ & $\bf{A_{sparse}}$ dünnbesetzt \\
  \hline
...
\IEEEPARstart{D}{ie} besondere Architektur der GPU führt zu besonderen Problemen
...
Bislang langsamer als CPU-Implementierung

\subsection{IDRS Gesamtalgorithmus}
...
\begin{figure}[htbp]
   ...
   \caption{Ein Platzhalter: Bastian hat GNUplot benutzt}%

\end{figure}



\section{Möglichkeiten für die weitere Optimierung}
...
Instrumentation-Code analog dem Tuning-Interface des Oracle-RDBMS \cite{oracle}. Die Idee besteht darin
